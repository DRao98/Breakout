...
class GameScene: SKScene, SKPhysicsContactDelegate
{
    ...
    func makeBall()
    {
        ...
        ball = SKSpriteNode(color: UIColor.yellow, size: CGSize(width: ballDiameter, height: ballDiameter))
    ...
    {
        brick = SKSpriteNode(color: UIColor.blue, size: CGSize(width: brickWidth, height: brickHight))
    ...
    func makeLives()
    {
        livesNode = SKLabelNode(text: "Lives: 3")
        livesNode.position = CGPoint(x: -100, y: frame.maxY - 50)
        
        if lives == 2
        {
            livesNode.text = "Lives: 2"
        }
        else if lives == 1
        {
            livesNode.text = "Lives: 1"
        }
